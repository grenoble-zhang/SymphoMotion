...
: "${CSV_PATH:?Set CSV_PATH to a CSV manifest with object trajectory samples.}"
...
OUTPUT_DIR=${OUTPUT_DIR:-outputs/object_control}
...
if [[ -n "${OBJ_INJECTOR_PATH:-}" ]]; then
  EXTRA_ARGS+=(--obj_injector_path "$OBJ_INJECTOR_PATH")
fi
...
accelerate launch --config_file "$ACCELERATE_CONFIG" --main_process_port "$MASTER_PORT" \
  ...
  --use_object_prompt \
  --normalize_object_to_first_frame \
  --max_entities "${MAX_ENTITIES:-2}" \
  --obj_cross_attn_interval "${OBJ_CROSS_ATTN_INTERVAL:-2}" \
  --obj_scale "${OBJ_SCALE:-1.0}" \
  --obj_traj_mid_dim "${OBJ_TRAJ_MID_DIM:-512}" \
  --max_text_tokens "${MAX_TEXT_TOKENS:-50}" \
